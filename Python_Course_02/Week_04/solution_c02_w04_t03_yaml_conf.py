...
def constructor_example_class(loader, node):
    value = loader.construct_mapping(node, deep=True)
    class_dict = {"!easy_level": EasyLevel, "!medium_level": MediumLevel, "!hard_level": HardLevel}
    level_class = class_dict[node.tag]
    lvl_map = level_class.Map()
    lvl_obj = level_class.Objects()
    lvl_obj.config = value
    level = {'map': lvl_map, 'obj': lvl_obj}
    return level


# регистрируем конструктор
yaml.add_constructor('!easy_level', constructor_example_class)
yaml.add_constructor('!medium_level', constructor_example_class)
yaml.add_constructor('!hard_level', constructor_example_class)
