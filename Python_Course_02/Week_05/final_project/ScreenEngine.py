...
colors = {
    ...
    "blue": (0, 0, 255, 255),
    "wooden": (153, 92, 0, 255),
    "light_wooden": (200, 152, 50, 255),
    "dark_wooden": (50, 12, 0, 255),
}
...
class GameSurface(ScreenHandle):
    def calculate_min_x_y(self):
        hero = self.game_engine.hero
        self.min_x = max(0, hero.position[0] - 3)
        self.min_y = max(0, hero.position[1] - 3)
    ...
    def draw_hero(self):
        self.calculate_min_x_y()
        self.game_engine.hero.set_screen_pos(self.min_x, self.min_y)
        self.game_engine.hero.draw(self)

    def draw_map(self):
        # DONE FIXME || calculate (min_x,min_y) - left top corner
        min_x, min_y = self.calculate_min_x_y()
...
class HelpWindow(ScreenHandle):
    def __init__(self, *args, **kwargs):
        ...
        self.data.append([" M ", "Open/close map"])
...
class MapWindow(ScreenHandle):
    ...
    def draw(self, canvas):
        ...
        if self.game_engine.show_map:
            pygame.draw.lines(self, (255, 0, 0, 255), True, [
                (0, 0), (500, 0), (500, 500), (0, 500)], 5)
            self.cell_size_x = size[0] / len(self.game_engine.map[0])
            self.cell_size_y = size[1] / len(self.game_engine.map)

            for i in range(len(self.game_engine.map)):
                for j in range(len(self.game_engine.map[i])):
                    color = colors["dark_wooden"] if self.game_engine.map[i][j] == Service.wall else colors["light_wooden"]
                    pygame.draw.rect(self, color,
                                     (j * self.cell_size_y, i * self.cell_size_x, self.cell_size_y, self.cell_size_x))
            ...
            for obj in self.game_engine.objects:
                ...
                pygame.draw.rect(self, color,
                                     (j * self.cell_size_y, i * self.cell_size_x, self.cell_size_y, self.cell_size_x))
            color = (0, 0, 255)
            j, i = self.game_engine.hero.position
            pygame.draw.rect(self, color,
                             (j * self.cell_size_y, i * self.cell_size_x, self.cell_size_y, self.cell_size_x))
