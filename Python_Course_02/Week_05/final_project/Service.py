...
def give_luck(engine, hero):
    engine.score += 0.1
    engine.hero = Objects.Fortune(hero)
    engine.notify("Fortune applied")
...
def fight_against_hero(enemy, engine, hero):
    hit = max(5, int(enemy.stats["strength"] * random.random()))
    hero.hp -= hit
    if hero.hp <= 0:
        ...
        return
    hero.exp += enemy.xp
    engine.notify(f"Enemy hits you: -{hit} HP")
    ups = hero.level_up()
    for message in ups:
...
# DONE FIXME add classes for YAML !empty_map and !special_map{}
